...
#include "oledfont_hw.h"
#include "string.h"
#include "stdlib.h"
#include "math.h"
#define OLED_ADDRESS 0x3c
const uint8_t oled_init_data[30] = {0X00, 0XAE, 0X00, 0X10, 0X40, 0X81,
...
{
    OLED_setXY(_x, _y) return oled_write(T, size)
}
void OLED_init();
int i = 0;
...
void oled_write(uint8_t data)
{
    if (current_font != 2)
    {
        if (data >= 32 && data <= (uint8_t)'z')
        {
            if (current_font)
            {
                i2c_send(9, (uint8_t *)F8X16[data - 32]);
                OLED_setXY(x, y + 1);
                i2c_send(9, (uint8_t *)(F8X16[data - 32] + 8));
                OLED_setXY(x + 8, y - 1);
            }
            else
            {
                i2c_send(7, (uint8_t *)F6x8[data - 32]);
                x = x + 6;
            }
        }
        else if (data == '\n')
    ...
    else
    {
        if (data != '\n')
        {
            i2c_send(17, (uint8_t *)(Hzk[data]));
            while (NC)
                ;
            OLED_setXY(x, y + 1);
            i2c_send(17, (uint8_t *)(Hzk[data] + 16));
            while (NC)
                ;
...
void OLED_setXY(uint8_t _x, uint8_t _y)
{
    uint8_t set_posion[4];
    set_posion[0] = 0x00;
    set_posion[1] = 0xb0 + _y;
    set_posion[2] = ((_x & 0xf0) >> 4) | 0x10;
    set_posion[3] = (_x & 0x0f) | 0x01;
    i2c_send(4, set_posion);
    while (NC)
        ;
    x = _x;
    y = _y;
...
void OLED_fill(uint8_t data)
{
    uint8_t cmd[4] = {0X00, 0xb0, 0x00, 0x10};
...
void OLED_init()
{
    i2c_send_init(OLED_ADDRESS,10);
    while (NC)
        ;
    i2c_send(30, (uint8_t *)oled_init_data);
    while (NC)
        ;
    OLED_setXY(0, 0);
}
